...
class CoNLL_UD():
    ...
    def __init__(self, sentence, log_level=LOG_LEVEL):
        '''
        Constructor
        '''
    ...
    def misc_attr(self, s):
        ...
        if s != '_':
            ...
            if 'cat' in tmp:
                # d['CAT'] = tmp['cat']
                d['CAT'] = self.clear_tag(tmp['cat'])
            if 'preds' in tmp:
                d['PREDS'] = [(i,p) for t in tmp['preds'].split(',') for i,p in [t.split(':')]]
            if 'args' in tmp:
                d['ARGS'] = [(i,p) for t in tmp['args'].split(',') for i,p in [t.split(':')]]
    ...
    def __repr__(self):
        ...
        nodes = sorted(self.graph.nodes(), key=lambda n: self.graph.node[n]['POSITION'])
        for n in nodes:
...
if __name__ == '__main__':
    ...
    deps = CoNLL_UD(sents[26].strip().split('\n'))
    print(deps)
    #for i in ['3', '8', '9', '10']:
