...
class CoNLL_UD_15(CoNLL_UD):
    '''
    without directions in the categories
    without direction features
    without xpostag
    a smart pruning for cuts: sort deps by distance
    dep cut at 10
    head dep cut at 10
    ...
    def __init__(self, sentence, tag_simplify=3, log_level=LOG_LEVEL):
        super(CoNLL_UD_15, self).__init__(sentence, tag_simplify, log_level)
        ...
        for w in self.nodes:
            ...
            if 'DEPENDENTS' in d:
                ...
                if feats['dep_count']>10:
                    ...
                    dependents = sorted(d['DEPENDENTS'], key=lambda x:(abs(d['POSITION']-self.graph.node[x]['POSITION']), self.graph.node[x]['POSITION']))[:10]
                else:
            ...
            feats['h_dep_count'] = 0
            if h != '0' and h != '_' and 'DEPENDENTS' in dh:
                ...
                if feats['h_dep_count']>10:
                    ...
                    self.logger.debug('used first 10 of ' + str(feats['dep_count']) + ' dependencies')
                    dependents = sorted(dh['DEPENDENTS'], key=lambda x:(abs(d['POSITION']-self.graph.node[x]['POSITION']), self.graph.node[x]['POSITION']))[:10]
                else:
                    dependents = dh['DEPENDENTS']
                for i,h_dep in enumerate(dependents):
    ...
    def print_feat_dict(self, d):
        keys = ['idx', 'pos', 'head_pos', 'head_rel', 
                    'h_head_pos', 'h_head_rel', 'dep_count', 'h_dep_count']
...
if __name__ == '__main__':
    ...
    with open('ud-treebanks-conll2017/UD_Russian-SynTagRus/ru_syntagrus-ud-train.conllu', 'r', encoding='utf-8') as sample:
        sents = sample.read().strip().split('\n\n')

    for sent in sents:
        print(sent)
        deps = CoNLL_UD_15(sent.strip().split('\n'), log_level=logging.DEBUG)
        print(deps)
        del deps
    ...
    with open('data_en/test_tiny.conllu', 'r', encoding='utf-8') as sample:
        sents = sample.read().strip().split('\n\n')
    deps = CoNLL_UD_15(sents[3].strip().split('\n'))
    print(deps)
    #for i in ['3', '8', '9', '10']:
    ...
    with open('data_tr/tr-ud-dev_tiny.conllu', 'r', encoding='utf-8') as sample:
        sents = sample.read().strip().split('\n\n')
    print(len(sents))
    deps = CoNLL_UD_15(sents[2].strip().split('\n'))
    print('-------------------')
    print(deps, end='')
    print('-------------------')
    deps.print_feats()
