...
class SupertaggerDep(object):
    '''
    classdocs
    '''


    def __init__(self, train=None, test=None, log_level=LOG_LEVEL, sent_cls=CoNLL_UD):
        '''
        Constructor
        '''
        ...
        if train:
            self.train = Data(train, cls=self.sent_cls)
            t0, t = t, time()
        ...
        if test:
            self.test = Data(test, cls=self.sent_cls)
            t0, t = t, time()
